...
model_name_lowhres =  'L0050N0376/Thermal_non_equilibrium'
...
def plot_gas_resolutions(ax, gas_type='HI', label = True):
    ...
    ax.plot(data_circ_rand[:,0], data_circ_rand[:,1], linestyle = 'solid', color='green', label = '25m6' if label else None)
    ...
    ax.plot(data_circ_face[:,0], data_circ_face[:,1], linestyle = 'solid', color='red', label = '12m5' if label else None)
    ...
    ax.plot(data_sph[:,0], data_sph[:,1], linestyle = 'solid', color='blue', label = '50m7' if label else None)
...
def plot_gas_all_methods(ax, gas_type='HI', label = True):
    ...
    data_circ_rand = np.loadtxt(dir_data + model_name + '/ProcessedData/' + gas_type + 'SFLaw_z0.0_circular_apertures_random_map.txt')
    data_circ_face = np.loadtxt(dir_data + model_name + '/ProcessedData/' + gas_type + 'SFLaw_z0.0_circular_apertures_face_on_map.txt')
    data_sph = np.loadtxt(dir_data + model_name + '/ProcessedData/' + gas_type + 'SFLaw_z0.0_spherical_apertures.txt')

    ax.plot(data_circ_rand[:,0], data_circ_rand[:,1], linestyle = 'solid', color='green', label = 'Proj. random' if label else None)
    ax.fill_between(data_circ_rand[:,0], data_circ_rand[:,1] - data_circ_rand[:,2], data_circ_rand[:,1] + data_circ_rand[:,3], facecolor = 'green', alpha=0.2, interpolate=True) 
    ax.plot(data_circ_face[:,0], data_circ_face[:,1], linestyle = 'solid', color='red', label = 'Proj. face-on' if label else None)
    ax.fill_between(data_circ_face[:,0], data_circ_face[:,1] - data_circ_face[:,2], data_circ_face[:,1] + data_circ_face[:,3], facecolor = 'red', alpha=0.2, interpolate=True)
