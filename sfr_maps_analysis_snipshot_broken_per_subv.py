...
fields ={'ExclusiveSphere/50kpc': ('StellarMass', 'StarFormationRate', 'HalfMassRadiusStars', 'AtomicHydrogenMass', 'MolecularHydrogenMass', 'KappaCorotStars', 'KappaCorotGas', 'DiscToTotalStellarMassFraction', 'MassWeightedMeanStellarAge', 'AngularMomentumStars', 'CentreOfMass', 'MostMassiveBlackHoleVelocity')}
...
(m30, sfr30, r50, mHI, mH2, kappacostar, kappacogas, disctotot, stellarage, Jstars, cp, v_at_cp) = h5data_groups
...
if(ngals > 0):
   ...
   if(subvolume == 0):
      ...
      gal_props = np.zeros(shape = (ngals, 15))
      gal_props[:,0] = sgn_in
   ...
   zgas_profile = np.zeros(shape = (ngals, nr))
   ...
   ################################# read particle data #####################################################
   fields_gn = {'PartType0': ('GroupNr_bound', 'Rank_bound')}
   fields = {'PartType0': ('Coordinates' , 'Masses', 'StarFormationRates', 'Temperatures', 'Densities', 'Velocities')}
   ...
   (coord, m, sfr, temp, dens, v) = h5data
   ...
   ################################# read particle data #####################################################
   if(snip_type == False): #data being read corresponds to a snapshot
   ...
   else: #data being read corresponds to a snipshot
         fields = {'PartType0': ('ReducedSpeciesFractions', 'TotalDustMassFractions', 'ReducedElementMassFractions')}
         print("Will read part type 0 properties")
         h5data = common.read_particle_colibre_single_subv(model_dir, snap_file, subvolume, fields)
         print("Have read particle 0 properties")
         speciesfrac, DustMassFracTot, elementmassfracs = h5data
         del(h5data) #release data

         speciesfrac2 = np.zeros(shape = (len(speciesfrac[:,0]), 2))
         speciesfrac2[:,0] = speciesfrac[:,0]
   ...
   for g in range(0,ngals):
       ...
       if(npartingal > 0):
          ...
          print("Mean x velocity (should be ~0):", np.mean(v_part0[:,0]))
          ...
          if (family_method == 'radial_profiles'):
              for i,r in enumerate(xr):
                  ...
                  if(len(dcentre[inr]) > 0):
                     ...
                     mz_inr = m_part0[inr] * elementmassfracs_part0[inr,1]
                     ...
                     if(len(mz_inr[coldp]) > 0):
                        mhdiff_inr_cold = mh_inr[coldp]
                        mz_inr_cold = mz_inr[coldp]
                        mpart_inr_cold = mpart_inr[coldp]
                        vr_inr_cold = vr_inr[coldp]
                        sth_inr2_cold = sth_inr2[coldp]
                        zgas_profile[g,i] = np.sum(mz_inr_cold) / np.sum(mpart_inr_cold)
                        coldgas_profile[g,i] = np.sum(mpart_inr_cold)
                        npart0_cold_profile[g,i] = len(mz_inr[coldp]) 
                     ...
                     del(temp_inr, dens_inr, mh_inr, mz_inr, sfr_inr, coldp) #release data
