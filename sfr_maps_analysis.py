...
for z in range(0, 1): #len(snap_files)):
    ...
    if(ngals > 0):
       ...
       if (family_method == 'radial_profiles'):
            ...
            for i,ri in enumerate(gr):
                for j,rj in enumerate(gr):
                    r_dist_centre[i * len(rj) + j] = np.sqrt(ri**2 + rj**2)
 
       ################################# read particle data #####################################################
       fields = {'PartType0': ('GroupNr_bound', 'Coordinates' , 'Masses', 'StarFormationRates', 'Temperatures', 'SpeciesFractions', 'ElementMassFractions', 'ElementMassFractionsDiffuse', 'DustMassFractions')}
       ...
       (sgnp, coord, m, sfr, temp, speciesfrac, elementmassfracs, elementmassfracsdiff, DustMassFrac) = h5data
              ...
              if (family_method == 'radial_profiles'):
                  for i,r in enumerate(xr):
                      ...
                      if(len(dcentre[inr]) > 0):
                         ...
                         coldp = np.where(temp_inr < 10**(4.5)) #select particles with temperatures cooler than 10^4.5K and calculate metallicity profiles with those particles only.
                      ...
                      if(len(dcentre_i[inr]) > 0):
                          ...
                          for j,rj in enumerate(gr):
                              ...
                              if(len(dcentre_j_inr[inrj]) > 0):
                                  #calculate profiles
                                  npart0_profile[g,i * len(rj) + j] = len(dcentre_j_inr[inrj])
                                  sfr_profile[g,i * len(rj) + j] = np.sum(sfr_inr[inrj])
                                  mHI_profile[g,i * len(rj) + j] = np.sum(mh_inr[inrj] * speciesfrac_part0[inr[inrj],1])
                                  mH2_profile[g,i * len(rj) + j] = np.sum(mh_inr[inrj] * speciesfrac_part0[inr[inrj],7] * 2) #factor 2 comes from H2 being two hydrogen atoms
                                  mdust_profile[g,i * len(rj) + j] = np.sum(mh_inr[inrj] * dust_part0[inr[inrj]])
                                  temp_inj = temp_inr[inrj]
                                  mh_inj = mh_inr[inrj]
                                  mo_inj = mo_inr[inrj] 
                                  mfe_inj = mfe_inr[inrj] 
                                  coldp = np.where(temp_inj < 10**(4.5)) #select particles with temperatures cooler than 10^4.5K and calculate metallicity profiles with those particles only.
                                  oh_profile[g,i * len(rj) + j] = np.sum(mo_inj[coldp]) / np.sum(mh_inj[coldp])
                                  feh_profile[g,i * len(rj) + j] = np.sum(mfe_inj[coldp]) / np.sum(mh_inj[coldp])
